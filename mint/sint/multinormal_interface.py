...
#        return 2.*np.log(noise)
    ...
    def get_sase(self, data, points = None, returnErrorQ = False):
        ...
        if self.quickNoiseQ is True:
            ...
            self.objective_acquisitions = np.mean(data) + np.random.randn(self.points) * stderr_of_mean
            self.objective_mean = np.mean(data)
            self.objective_stdev = sample_stdev
            
            if self.stat_name == 'Mean' or self.stat_name == 'Median':
                self.statistic = self.objective_mean
            elif self.stat_name == 'Standard deviation':
                self.statistic = self.objective_stdev
            elif self.stat_name == 'Median deviation':
                self.statistic = self.objective_stdev * np.sqrt(2)*erfinv(0.5)
            elif self.stat_name == 'Max':
                self.statistic = self.objective_mean + self.objective_stdev * np.max(np.random.randn(self.points))
            elif self.stat_name == 'Min':
                self.statistic = self.objective_mean + self.objective_stdev * np.min(np.random.randn(self.points))
            elif self.stat_name == 'average of points > mean':
                self.statistic = self.objective_mean + np.sqrt(2./np.pi) * self.objective_stdev
            elif self.stat_name == '80th percentile':
                # percentile cut
                pvalue = 0.8 # pvalue = percentile/100.
                zscore = np.sqrt(2) * erfinv(2*pvalue-1)
                self.statistic = self.objective_mean + zscore * self.objective_stdev
            ...
            data = np.array(self.f(self.x)) # MultinormalInterface returns self.points entries per point
            
            self.objective_acquisitions = data
            self.objective_mean = np.mean(self.objective_acquisitions)
            self.objective_stdev = np.std(self.objective_acquisitions)
            #data = self.f(self.x)
            ...
            if self.stat_name == 'Median':
                self.statistic = np.median(self.objective_acquisitions)
            elif self.stat_name == 'Standard deviation':
                self.statistic = self.objective_stdev
            elif self.stat_name == 'Median deviation':
                median = np.median(self.objective_acquisitions)
                self.statistic = np.median(np.abs(self.objective_acquisitions-median))
            elif self.stat_name == 'Max':
                self.statistic = np.max(self.objective_acquisitions)
            elif self.stat_name == 'Min':
                self.statistic = np.min(self.objective_acquisitions)
            elif self.stat_name == '80th percentile':
                self.statistic = np.percentile(self.objective_acquisitions,80)
            elif self.stat_name == 'average of points > mean':
                self.statistic = np.mean(self.objective_acquisitions[self.objective_acquisitions>self.objective_mean])
            elif self.stat_name == '20th percentile':
                self.statistic = np.percentile(self.objective_acquisitions,20)
            else:
                self.stat_name = 'Mean'
                self.statistic = self.objective_mean
            
        print self.stat_name, ' of ', self.objective_acquisitions.size, ' points is ', self.statistic, ' and standard deviation is ', self.objective_stdev
        ...
        return self.statistic, self.objective_stdev
...
#        for pv in self.pvs: print(pv)
    ...
    def recordData(self, objective_func_pv, objective_func, devices):
        ...
        self.data[objective_func_pv] = [] #detector mean array for compat
        self.data['DetectorAll'] = [] #detector acquisition array
        self.data['DetectorStat'] = [] #detector statistic array
    ...
    def saveData(self, objective_func_pv, objective_func, devices, name_opt, norm_amp_coeff):
        ...
        self.data['DetectorMean']         = str(objective_func_pv.replace(":","_")) # reminder to look at self.data[objective_func_pv]
        self.data["NormAmpCoeff"]         = norm_amp_coeff
        self.data["niter"]                = objective_func.niter

        #save data
        import simlog
        self.last_filename = simlog.save("OcelotScan",self.data,path='default')#self.save_path)
